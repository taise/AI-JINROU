REVOTEACT<- function(PUBLICLOG,PRIVATELOG){
  ...
  BLACK <- unique(as.character(PUBLICLOG$MSG2[
    (as.character(PUBLICLOG$TYPE) == "SPREPORTA") & (as.character(PUBLICLOG$MSG1) == "人狼")
    ]))
  
  # 再投票対象者の名前を取得
  TSURI <- as.character(PUBLICLOG$MSG2[
    PUBLICLOG$TYPE=="REVOTE" & PUBLICLOG$day == max(PUBLICLOG$day)
  ])
  # 初期値：エラー回避用(自分を釣る)
  vote <- MYNAME
